...
eval env (EIf p t f)		= helper (eval env p) t f
    where
	helper (VBool True) tr fa = (eval env tr)
	helper (VBool False) tr fa = (eval env fa)
	helper _ tr fa = throw (Error "type error")
eval env (ELet id e1 e2)  	= eval env' e2
    where
	v   = eval env e1
	env'= (id,v):env
eval env (ELam fun e1) 		= VClos env fun e1
eval env (EApp fun e1) 		= eval bodyEnv body
    where
...
--------------------------------------------------------------------------------
evalOp Plus (VInt val1) (VInt val2) = VInt (val1 + val2)
evalOp Plus _ _ = throw (Error "type error")
evalOp Minus (VInt val1) (VInt val2) = VInt (val1 - val2)
evalOp Minus _ _ = throw (Error "type error")
evalOp Mul (VInt val1) (VInt val2) = VInt (val1 * val2)
evalOp Mul _ _ = throw (Error "type error")

evalOp Eq (VInt val1) (VInt val2) = VBool (val1 - val2 == 0)
evalOp Eq _ _ = throw (Error "type error")
evalOp Eq (VBool val1) (VBool val2) = VBool (val1 == val2)
evalOp Eq _ _ = throw (Error "type error")
evalOp Ne (VInt val1) (VInt val2) = VBool (val1 - val2 /= 0)
evalOp Ne _ _ = throw (Error "type error")
evalOp Ne (VBool val1) (VBool val2) = VBool (val1 /= val2)
evalOp Ne _ _ = throw (Error "type error")

evalOp Lt (VInt val1) (VInt val2) = VBool (val1 < val2)
evalOp Lt _ _ = throw (Error "type error")
evalOp Le (VInt val1) (VInt val2) = VBool (val1 <= val2)
evalOp Le _ _ = throw (Error "type error")

evalOp And (VBool val1) (VBool val2) = VBool (val1 && val2)
evalOp And _ _ = throw (Error "type error")
evalOp Or (VBool val1) (VBool val2) = VBool (val1 || val2)
...
--------------------------------------------------------------------------------
lookupId id ((k,v):rest) 
    | id == k = v
    | rest == [] = throw (Error ("unbound variable: " ++ id))
